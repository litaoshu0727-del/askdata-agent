...
def build_metric_glossary(business_meta: Dict[str, Any] | None, max_chars: int = 6000) -> str:
    """
    把业务元数据压成一份指标口径表。

    "客单价怎么算的"这类问题属于 data_qa，不该查库；
    但它需要知道"客单价"对应哪个字段、口径是什么——这些信息都在 business_meta 里，
    而 data_qa 原本只拿得到会话记忆，看不到 Schema，只能回答"上下文里没有定义"。

    这里只取字段名、别名和业务含义，不带样例值和索引文本，控制住 Prompt 体积。
    """
    ...
    for table_name, table_meta in business_meta.items():
        for column_name, column_meta in (table_meta.get("columns") or {}).items():
            description = (column_meta.get("description") or "").strip()
            aliases = column_meta.get("aliases") or []

            if not description and not aliases:
                continue

            alias_text = f"（业务上也叫：{'、'.join(aliases)}）" if aliases else ""
            lines.append(f"- {table_name}.{column_name}{alias_text}：{description}")

    glossary = "\n".join(lines)

    if len(glossary) > max_chars:
        glossary = glossary[:max_chars] + "\n……（字段过多，已截断）"
...
class ModelDataQAClient:
    ...
    def __init__(self, client, business_meta: Dict[str, Any] | None = None):
        self.client = client
        self.metric_glossary = build_metric_glossary(business_meta)
...
回答指标口径类问题时，优先依据下面的指标口径表，
指出该指标对应哪张表的哪个字段，以及它的业务含义。
{glossary_block}
会话上下文：
{context_text or '无'}
