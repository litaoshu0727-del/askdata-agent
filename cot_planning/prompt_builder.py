...
7. 若Schema确实不存在所需的表或字段，把该步骤的"数据库"一项填成"缺失"，
   并在"处理对象"中写明缺少哪个字段或哪张表。
8. 严禁用语义不同的字段替代用户所问的指标。
   用户问"物流签收时效"，而Schema只有支付时间和确认收货时间时，
   两者之差包含了商家备货时间，属于不同口径，必须判定为缺失。
   用户问"具体年龄"，而Schema只有年龄段时，年龄段不等于年龄，同样判定为缺失。
