...
class CotPlanner:
    ...
    def parse_steps(self, text: str) -> List[CotPlanningStep]:
        ...
        # 分隔符写成可选，并且允许全角/半角混用。
        #
        # 原先要求每个字段后面必须跟一个半角逗号，模型只要在句尾用了句号
        # （"……最后返回对应用户信息。" 后面直接换行写 输出目标），
        # 整个四元组就匹配不上，parse_steps 静默返回空列表，
        # 表现为 CoT 看着完全正常、却一条 SQL 都没生成。
        #
        # 这里改成用下一个标签名当锚点：字段内容非贪婪匹配到下一个标签为止，
        # 中间的逗号有就吃掉、没有也不影响。
        pattern = re.compile(
            r"步骤(?P<step_no>\d+)[：:]\s*"
            r"\(\s*"
        ...
        if not steps and text.strip():
            # 模型给了输出、却一个四元组都没解析出来。
            # 这时 run() 里的 for cot_step in cot_result.steps 一次都不会进，
            # 整条链路会安静地停在这里，不生成任何 SQL。
            emit(
                Codes.COT_PARSE_EMPTY,
                "CoT 有输出但未解析出四元组，本次不会生成任何 SQL，请检查模型输出格式",
                输出长度=len(text.strip()),
                输出预览=text.strip().replace("\n", " ")[:80],
            )
