...
class VectorIndex:
    ...
    def search(self, query: str, top_k: int = 5) -> List[Tuple[int, float]]:
        ...
        #
        # errstate 是为了屏蔽 macOS 上的虚假警告：numpy 2.x 搭配 Apple Accelerate
        # BLAS 时，矩阵乘法的 SIMD 内核会置位浮点异常标志，numpy 读到后会报
        # divide by zero / overflow / invalid value，但输出矩阵里并没有 NaN 或 Inf，
        # 计算结果完全正确。这里只屏蔽告警，不改变任何数值。
